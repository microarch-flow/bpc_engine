...
def render_chart(
    ...
    ax.set_title(
        f"LLM decode data-movement / compute ratio | Batch = {batch}",
        loc="left",
        fontsize=18,
        pad=26,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config-dir",
        type=Path,
        default=PROJECT_ROOT / "configs" / "4bit",
        help="Directory containing model JSON configs.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "outputs" / "decode_ratio" / "4bit",
