...
当前阶段仍是研究数据准备，尚未进入趋势拟合和芯片指标预测。跨年度样本清单见
...
- `studies/decode_trend/releases/*/`是可复现生成物，已被Git忽略，不应提交。
- 生成版本包含20个模型、3357行结果、完整`source_snapshot/`和`SHA256SUMS`。
- 后续统计应先生成版本目录，再读取其中的`data/decode_results.csv`。
...
当前状态是：**20个模型的数据准备、P3审计、字段字典和正式数据生成链均已完成**。生成目录不提交Git；用户计划在新会话中生成数据并开始统计分析。

P2 已建立 [字段能力矩阵](docs/decode_trend_field_capability_matrix.md)。结论不是重写计算引擎：核心 Step/Token FLOPs、流量和 Cache 公式已具备；主要缺口是统一模型事实、显式外推、logical-HBM 总量排除 Activation、研究级导出和审计状态。
...
1. 先按`studies/decode_trend/releases/README.md`生成数据，再从版本目录的`data/`读取。
2. 先决定P3 `partially_supported`项进入中心拟合还是仅进入敏感性分析。
3. 补充模型能力控制字段，再做算法效率趋势；不能只按年份拟合所有模型。
4. 分开分析行业需求包络、算法效率和部署采用，不能合并成一条曲线。

不要直接开始拟合趋势。当前样本量少，必须先把字段、扫描网格和缺失值规则对齐。

## 9. 当前工作区状态提醒

工作区不是干净状态。已有改动和未跟踪文件大多是本轮长期研究的一部分或历史残留，不能随手清理、reset、checkout。

已知状态包括：

- `configs/2022/` 至 `configs/2026/` 仍显示为 untracked；
- `AGENTS.md`、`HANDOFF.md`、趋势研究文档也可能显示为 untracked；
- `decode_engine/__pycache__/` 有被错误追踪的 modified pyc；
- `tests/test_prefill.py` 有既有修改；
- 不要运行 `git reset --hard`、`git checkout --` 或清理 untracked，除非用户明确要求。
...
> 20 个样本、机器事实清单、双容量口径和批量运行器已完成；当前先验收 P2 事实质量，再做 P3 引擎覆盖矩阵，不能直接跳到趋势拟合。
